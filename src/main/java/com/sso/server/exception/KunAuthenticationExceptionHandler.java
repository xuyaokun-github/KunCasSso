 ...
 * @since 4.0.0
 */
public class KunAuthenticationExceptionHandler {
	...
	static {
		DEFAULT_ERROR_LIST.add(KunLoginException.class);
		DEFAULT_ERROR_LIST.add(KunSingleLoginException.class);
			...
			for (final Class<? extends Exception> kind : this.errors) {
				for (final Class<? extends Exception> handlerError : e.getHandlerErrors().values()) {
					if (handlerError != null && handlerError.equals(kind)) {
						...
						if(handlerErrorName.equals(KunLoginException.class.getSimpleName())){
							if(KunLoginException.errMsg.get() != null){
								messageContext.addMessage(builder.error().code(messageCode).args(KunLoginException.errMsg.get()).build());
								KunLoginException.errMsg.remove();
							}else{
								messageContext.addMessage(builder.error().code(messageCode).build());
							}
							return handlerErrorName;
						} else if (handlerErrorName.equals(KunSingleLoginException.class.getSimpleName())) {
							if(KunSingleLoginException.errMsg.get() != null){
								messageContext.addMessage(builder.error().source(KunSingleLoginException.errMsg.get().getErrCode()).code(KunSingleLoginException.errMsg.get().getErrMsg()).build());
								KunSingleLoginException.errMsg.remove();
							}else{
								messageContext.addMessage(builder.error().code(messageCode).build());
							}
							return handlerErrorName;
