...
$.formValidator = 
{
		...
		else
		{
			...
			else
			{
				...
				if(stype == "password" || stype == "text" || stype == "file")
				{
					jqobj = $(elem);
					var oldClass = "";
					if(jqobj.attr("oldClass") == undefined && jqobj.attr("class") != undefined){
						jqobj.attr("oldClass",jqobj.attr("class"));
					}
					oldClass = jqobj.attr("oldClass");
					if(onShowClass!="" && showclass == "onShow"){jqobj.removeClass().addClass(oldClass+" "+onShowClass)};
					if(onFocusClass!="" && showclass == "onFocus"){jqobj.removeClass().addClass(oldClass+" "+onFocusClass)};
					if(onCorrectClass!="" && showclass == "onCorrect"){jqobj.removeClass().addClass(oldClass+" "+onCorrectClass)};
					if(onErrorClass!="" && showclass == "onError"){jqobj.removeClass().addClass(oldClass+" "+onErrorClass)};
				}
			}
		}
	},
	...
	hideAutoTip : function(validatorGroup)
	{
		if(validatorGroup == undefined) validatorGroup = "1";
		var initConfig = $.formValidator.getInitConfig(validatorGroup);
		$.each(initConfig.validObjects,function()
